    ...
    selectTargetOptions() {
      ...
      for (const option of this.targetOptions) {
        selectTargetOptions.push({
          label: this.t(`kubearmorpolicy.rules.${ option }.label`),
          value: option,
        });
      }
    ...
    <div class="row mb-20">
      <div class="col span-6">
        ...
        <LabeledSelect
          ...
          :mode="mode"
          :tooltip="targetType === TARGET_OPTIONS.NAMESPACE_AND_POD_SELECTOR ? t('kubearmorpolicy.selectors.matchingNamespacesAndPods.tooltip') : null"
          :options="selectTargetOptions"
          :multiple="false"
          :label="t('kubearmorpolicy.rules.type')"
        />
      </div>
    </div>
    <div v-if="targetType === TARGET_OPTIONS.IP_BLOCK">
      <div class="row">
        <div class="col span-6">
          <LabeledInput
            ...
            :mode="mode"
            :placeholder="t('kubearmorpolicy.rules.ipBlock.cidr.placeholder')"
            :label="t('kubearmorpolicy.rules.ipBlock.cidr.label')"
          />
        </div>
      </div>
      <div
      ...
      >
        <div class="col span-12">
          <Banner color="error">
            <t k="kubearmorpolicy.rules.ipBlock.invalidCidr" />
          </Banner>
        </div>
      </div>
      <div class="row mt-20">
        <div class="col span-12">
          <ArrayList
            ...
            :add-label="t('kubearmorpolicy.rules.ipBlock.addExcept')"
            :mode="mode"
            :show-header="true"
            :value-label="t('kubearmorpolicy.rules.ipBlock.exceptions')"
            :value-placeholder="t('kubearmorpolicy.rules.ipBlock.cidr.placeholder')"
          />
        </div>
      </div>
      <div
      ...
      >
        <div class="col span-12">
          <Banner color="error">
            <t k="kubearmorpolicy.rules.ipBlock.invalidExceptionCidrs" />{{ invalidCidrs.join(', ') }}
          </Banner>
        </div>
      </div>
    </div>
      ...
      <div class="row">
        <div class="col span-12">
          <Banner color="success">
            <span v-clean-html="t('kubearmorpolicy.selectors.matchingPods.matchesSome', matchingPods)" />
          </Banner>
        </div>
      </div>
      <div class="row mb-0">
    ...
    <div v-if="targetType === TARGET_OPTIONS.NAMESPACE_SELECTOR">
      <div class="row">
        <div class="col span-12">
          <Banner color="success">
            <span
              v-clean-html="t('kubearmorpolicy.selectors.matchingNamespaces.matchesSome', matchingNamespaces)"
    ...
    <div v-if="targetType === TARGET_OPTIONS.NAMESPACE_AND_POD_SELECTOR">
        ...
        <div class="col span-12">
          <Banner color="success">
            <span
              ...
              v-clean-html="t('kubearmorpolicy.selectors.matchingPods.matchesSome', matchingPods)"
            />
            <span
              v-else
              v-clean-html="t('kubearmorpolicy.selectors.matchingNamespacesAndPods.matchesSome', matchingNamespacesAndPods)"
            />
          </Banner>
        </div>
      </div>
      <div class="row mb-0">
        <div class="col span-1 namespace-pod-rule">
          <span class="label">
            {{ t('kubearmorpolicy.rules.namespace') }}
          </span>
        </div>
